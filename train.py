    ...
    def get_equivalent_data(self, play_data):
        ...
        for state, mcts_probabilities, winner in play_data:
            for i in [1, 2, 3, 4]:
                ...
                equivalent_state = np.array([np.rot90(s, 0 if 0 < k < 3 else i) for k, s in enumerate(state)])
                ...
                # flip horizontally
                equivalent_state = np.array([s if 0 < k < 3 else np.fliplr(s) for k, s in enumerate(equivalent_state)])

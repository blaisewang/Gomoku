...
def state_function_thread(chess_copy, moves, args) -> ({}, int):
    global move, chess
    move = moves
    x, y = args
    state_dictionary = {}
    ...
    if state_lock.acquire():
        chess = chess_copy
        state_dictionary = get_state()
        reward = get_reward(x, y)
        state_lock.release()

    return state_dictionary, reward


def get_state() -> {}:
    global one_dimensional_opponent_pattern_list
    global one_dimensional_opponent_winning_pattern_list, two_dimensional_opponent_winning_pattern_list
        ...
        for i in range(4, 19):
            for j in range(4, 19):
                if chess[i][j] == player:
                    ...
                    for key, pattern, need_reverse, left_offset, _ in one_dimensional_opponent_pattern_list:
                        number = one_dimensional_pattern_match(pattern, need_reverse, i, j, left_offset)
                        if number > 0:
                            pattern_dictionary[key] += number
...
def get_reward(x: int, y: int) -> int:
    ...
    for _, pattern, left_offset in one_dimensional_opponent_winning_pattern_list:
    ...
    for _, pattern, need_rotate, (anchor_x, anchor_y) in two_dimensional_opponent_winning_pattern_list:
    ...
    for _, pattern, need_reverse, left_offset, expect_reward in one_dimensional_opponent_pattern_list:
